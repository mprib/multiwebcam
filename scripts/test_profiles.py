...
from multiwebcam.profiles.profile import SourceProfile
...
def main():
    ...
    for i, opts in enumerate(discovered):
        ...
        repo.save(profile)
        print(f"  Saved: source_id={profile.source_id}, label={profile.label}")
    ...
    for profile in loaded:
        print(f"  source_id={profile.source_id}, label={profile.label}, bus_info={profile.bus_info}")

    print("\n=== Matching against discovery ===")
    for opts in discovered:
        profile = repo.get_by_bus_info(opts.bus_info)
        if profile:
            print(f"  {opts.path} -> source_id={profile.source_id} ({profile.label})")
        else:
    ...
    if loaded:
        ...
        repo.save(updated)
        print(f"  Updated source_id={first.source_id} -> label={updated.label}, resolution={updated.resolution}")

        # Reload and verify
        reloaded = repo.get_by_source_id(first.source_id)
        assert reloaded is not None
        print(f"  Verified: label={reloaded.label}, resolution={reloaded.resolution}")

    print("\n=== Testing next_source_id ===")
    next_id = repo.next_source_id()
    print(f"  Next available source_id: {next_id}")
